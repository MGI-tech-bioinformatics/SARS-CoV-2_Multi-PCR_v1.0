...
=head1 Version
  ...
  Mender: Lu Changxin, luchangxin@genomics.org.cn
  Version: 8.1,  Date: 2012-01-04
...
=head1 Exmple
  ...
  8.recommend combination of usages for common applications under new strategy
  perl qsub-sge.pl --queue bc.q --resource vf=1.9G -maxjob 10 --reqsub --pro_code gagtest ./work.sh
...
my ($Queue, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub,$Resource,$node,$Job_prefix,$Verbose, $Help,$Pro_code);
GetOptions(
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
	"interval:i"=>\$Interval,
...
while (@Shell) {
	...
	$job_cmd .= "-P $Pro_code " if (defined $Pro_code);##set -P,added by luchx
    $job_cmd .= "-l $Resource " if(defined $Resource); ##set resource
        $job_cmd .="-l $node " if(defined $node); ##set the compute-node
	##warn $job_cmd;
